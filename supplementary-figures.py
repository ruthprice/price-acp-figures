# supplementary-figures.py 
# Make figures for appendices
# RP Jan 2023
# =====================================================================
...
if verbose:
    print('\nMaking PDFs..')
n_pdf_bins = 25
hist_obs, hist_model, pdf_bins = aero.ao2018_melt_freeze_pdfs(n_pdf_bins, figB1_suites, N, bin_edges, times)
pdf_bins_mid = [0.5*(X[1:] + X[:-1]) for X in pdf_bins]

# ---------------------------------------------------------------------
# LOAD MODEL DATA FOR FIGURE D1
model_output_path = '/gws/nopw/j04/asci/rprice/ukca_output/'
for suite in figD1_suites:
    if verbose:
        print('\nLoading output from {}..'.format(suite))
    N[suite], bin_edges[suite], times[suite] = aero.get_ao2018_aerosol_conc(model_output_path, suite)
if verbose:
    print('\nMaking PDFs..')
n_pdf_bins = 25
hist_obs, hist_model, pdf_bins = aero.ao2018_melt_freeze_pdfs(n_pdf_bins, figD1_suites, N, bin_edges, times, hist_model=hist_model, pdf_bins=pdf_bins)
pdf_bins_mid = [0.5*(X[1:] + X[:-1]) for X in pdf_bins]

# ---------------------------------------------------------------------
# LOAD DATA FOR FIGURE D2
if verbose:
    print('\nLoading data for figure D2..')
# load observations
...
# ---------------------------------------------------------------------
# LOAD MODEL OUTPUT FOR FIG D2
if verbose:
    print('\nLoading output for fig D2..')
model_output_path = '/gws/nopw/j04/asci/rprice/ukca_output/'

model_N = {}
for s,suite in enumerate(figD1_suites):
    if verbose:
        print(suite)
    model_N[suite], heights = aero.calculate_ascos_aerosol_conc(model_output_path, suite, ascos_flight_data)

# ---------------------------------------------------------------------
# LOAD DATA FOR FIGURE D3
if verbose:
    print('\nLoading data for figD3..')
...
# ---------------------------------------------------------------------
# LOAD MODEL OUTPUT FOR FIGURE D3
if verbose:
    print('\nLoading output for figD3..')
model_output_path = '/gws/nopw/j04/asci/rprice/ukca_output/'
number_conc = {}
number_conc_STP = {}
radius = {}
...
# ---------------------------------------------------------------------
# LOAD MODEL DATA FOR FIGURE E1
model_output_path = '/gws/nopw/j04/asci/rprice/ukca_output/'
for suite in figE1_suites:
    if verbose:
        print('\nLoading output from {}..'.format(suite))
    N[suite], bin_edges[suite], times[suite] = aero.get_ao2018_aerosol_conc(model_output_path, suite)
if verbose:
    print('\nMaking PDFs..')
n_pdf_bins = 25
hist_obs, hist_model, pdf_bins = aero.ao2018_melt_freeze_pdfs(n_pdf_bins, figE1_suites, N, bin_edges, times, hist_model=hist_model, pdf_bins=pdf_bins)
pdf_bins_mid = [0.5*(X[1:] + X[:-1]) for X in pdf_bins]
...
# ---------------------------------------------------------------------
# FIGURE B1: time series of aerosol concentrations during AO2018
if verbose:
    print('\nMaking figure B1..')
...
ts.plot_time_series_pdfs(figB1_suites, N, times, 
...
# ---------------------------------------------------------------------
# FIGURE D1: time series of aerosol concentrations during AO2018
if verbose:
    print('\nMaking figure D1..')
...
obs_T = [ufp_running_mean_times, dmps_running_mean_times, dmps_running_mean_times]
obs_stdev = [ufp_std_devs, dmps_N15_100_std_devs, dmps_N100_500_std_devs]
ts.plot_time_series_pdfs(figD1_suites, N, times, 
                         obs_N, obs_T, obs_stdev, 
                         hist_obs, hist_model, pdf_bins_mid,
                         'figures/figD1.pdf')
if verbose:
...
# ---------------------------------------------------------------------
# FIGURE E1: time series of aerosol concentrations during AO2018
if verbose:
    print('\nMaking figure E1..')
...
obs_T = [ufp_running_mean_times, dmps_running_mean_times, dmps_running_mean_times]
obs_stdev = [ufp_std_devs, dmps_N15_100_std_devs, dmps_N100_500_std_devs]
ts.plot_time_series_pdfs(figE1_suites, N, times, 
                         obs_N, obs_T, obs_stdev, 
                         hist_obs, hist_model, pdf_bins_mid,
                         'figures/figE1.pdf')
ts.plot_time_series_pdfs(figE1_suites[:2], N, times, 
                         obs_N, obs_T, obs_stdev, 
                         hist_obs, hist_model, pdf_bins_mid,
                         'figures/figE1a.pdf')
ts.plot_time_series_pdfs(figE1_suites[2:4], N, times, 
                         obs_N, obs_T, obs_stdev, 
                         hist_obs, hist_model, pdf_bins_mid,
                         'figures/figE1b.pdf')
# ts.plot_time_series_pdfs(figE1_suites[4:6], N, times, 
#                          obs_N, obs_T, obs_stdev, 
#                          hist_obs, hist_model, pdf_bins_mid,
#                          'figures/figE1c.pdf')
# ts.plot_time_series_pdfs(figE1_suites[6:], N, times, 
#                          obs_N, obs_T, obs_stdev, 
#                          hist_obs, hist_model, pdf_bins_mid,
#                          'figures/figE1d.pdf')
if verbose:
